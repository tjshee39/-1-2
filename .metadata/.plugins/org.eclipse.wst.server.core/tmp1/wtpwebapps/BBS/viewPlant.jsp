...
<%@ page import="bbsplant.BbsPlantDAO" %>
...
<!-- 글내용 보기 -->
	...
	<%   //로그인 된 사람들의 정보를 담을 수 있도록 만듦
		...
		if (request.getParameter("bbsPlantID") != null) {  //bbsID라는 매개변수가 존재한다면
			bbsPlantID = Integer.parseInt(request.getParameter("bbsPlantID"));
		}
		...
		if (bbsPlantID == 0) {
			...
			script.println("location.href = 'bbsPlant.jsp");   //bbs.jsp로 넘어가기
			script.println("</script>");
		}
		
		BbsPlant bbsplant = new BbsPlantDAO().getBbsPlant(bbsPlantID);  //구체적인 정보를 bbs라는 인스턴스에 담아줌
	%>
		...
		<div class="row">  <!-- table-striped: 게시판의 글 목록들이 홀/짝수 번갈아가면서 색상 변경 -->
			...
			<tbody>
				<tr>
					...
					<td colspan="2"><%= bbsplant.getBbsPlantTitle().replaceAll(" ", "&nbsp;")
					.replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br/>") %></td>
							<!-- <script>가 삽입될 수 있는 문제 방지 -->
				</tr>	    <!-- input: action페이지에 전송 -->
				<tr>
					<td>작성자</td>
					<td style="width: 20%;"><%= bbsplant.getUserID() %></td>
					<td>조회수</td>
					<td style="width: 20%;"><%= bbsplant.getBbsPlantHit() %></td>
				</tr>
				<tr>
					<td colspan="2">작성일자</td>
					<td colspan="2" style="width: 20%;"><%= bbsplant.getBbsPlantDate().substring(0, 11) 
					+ bbsplant.getBbsPlantDate().substring(11, 13) 
					+ "시" + bbsplant.getBbsPlantDate().substring(14, 16) + "분"%></td>
				</tr>
				<tr>
					<td colspan="4">내용</td>
				</tr>
				<tr>
					<td colspan="4" style="min-height: 200px; text-align: center; margin-left: auto; margin-right: auto;">
						<%= bbsplant.getBbsPlantContent().replaceAll(" ", "&nbsp;")
						.replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br/>") %>
					</td>
					<!-- 특수문자 대체 대체 -->
				</tr>
			...
			<a href = "bbsPlant.jsp" class = "btn btn-primary" style="background-color: #b7b7b7; border-color: #b7b7b7;">목록</a>
			<c:if test="${userID eq '관리자' }">
				<a href="updateLife.jsp?bbsLifeID=<%= bbsPlantID %>" class = "btn btn-primary" style="background-color: #b7b7b7; border-color: #b7b7b7;">수정</a>
				<a onclick="return confirm('삭제하시겠습니까?')" href="deleteLifeAction.jsp?bbsLifeID=<%= bbsPlantID %>" class = "btn btn-primary" style="background-color: #b7b7b7; border-color: #b7b7b7;">삭제</a>
			</c:if> 
		</div>
	</div>
