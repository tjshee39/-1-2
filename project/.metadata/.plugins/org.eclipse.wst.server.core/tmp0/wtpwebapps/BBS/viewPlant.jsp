		...
		<div class="row">  <!-- table-striped: 게시판의 글 목록들이 홀/짝수 번갈아가면서 색상 변경 -->
			...
			<tbody>
				<tr>
					...
					<td style="width: 20%;"><%= bbsplant.getBbsPlantTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;")
				...
				<tr>
					<td>작성일자</td>
					<td style="width: 20%;"><%= bbsplant.getBbsPlantDate().substring(0, 11) + bbsplant.getBbsPlantDate().substring(11, 13) 
					+ "시" + bbsplant.getBbsPlantDate().substring(14, 16) + "분"%></td>
				</tr>
				<tr>
					<td>내용</td>
					<td style="min-height: 200px; text-align: left;">
						<%= bbsplant.getBbsPlantContent().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;")
						.replaceAll(">", "&gt;").replaceAll("\n", "<br/>") %>
					</td>
					<!-- 특수문자 대체 대체 -->
				</tr>
			...
			<a href = "bbsPlant.jsp" class = "btn btn-primary">목록</a>
			<%
				if (userID != null && userID.equals(bbsplant.getUserID())) {  //본인이 쓴 글이라면 수정, 삭제 가능
			%>
					<a href="updatePlant.jsp?bbsPlantID=<%= bbsPlantID %>" class = "btn btn-primary">수정</a>
					<a onclick="return confirm('삭제하시겠습니까?')" href="deletePlantAction.jsp?bbsPlantID=<%= bbsPlantID %>" 
