...
<!-- 글 수정 -->
	...
	<%   //로그인 된 사람들의 정보를 담을 수 있도록 만듦
		...
		BbsLife bbslife = new BbsLifeDAO().getBbsLife(bbsLifeID);  //넘어온 bbsID로 해당 글을 가져옴
		if (!userID.equals(bbslife.getUserID())) {  //현재 유저가 글 작성자가 아니라면
			PrintWriter script = response.getWriter();
			script.println("<script>");
	...
	<div class="container">  <!-- 특정한 내용을 담을 수 있는 container 지정 -->
		<div class="row">  <!-- table-striped: 게시판의 글 목록들이 홀/짝수 번갈아가면서 색상 변경 -->
			<form method="post" action="updateLifeAction.jsp?bbsLifeID=<%= bbsLifeID %>">  <!-- bbsID가 updateAction.jsp로 보내짐 -->
				<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
					...
					<tbody>
						<tr>
							<td><input type = "text" class="form-control" placeholder="글 제목" name="bbsLifeTitle" maxlength="50" 
							value=<%= bbslife.getBbsLifeTitle() %>></td>  <!-- 수정하기 전의 제목 보여줌 -->
						</tr>	   
						<tr>
							<td><textarea class="form-control" placeholder="글 내용" name="bbsLifeContent" maxlength="2048" 
							style="height: 350px;"><%= bbslife.getBbsLifeContent() %></textarea></td>
							<!-- 수정하기 전의 내용 보여줌 -->
						</tr>
					</tbody>
				</table>
