...
<%@ page import="bbsinsect.BbsInsectDAO" %>  <!-- bbs패키지에 있는 BbsDAO -->
...
<body>
	...
	<div class="container">  <!-- 특정한 내용을 담을 수 있는 container 지정 -->
		<div class="row">  <!-- table-striped: 게시판의 글 목록들이 홀/짝수 번갈아가면서 색상 변경 -->
			...
			<tbody>
				<%
					BbsInsectDAO bbsInsectDAO = new BbsInsectDAO();
					ArrayList<BbsInsect> list = bbsInsectDAO.getList(pageNumber);
					for (int i = 0; i < list.size(); i++) {
				%>
				<tr>
					<td><%= list.get(i).getBbsInsectID() %></td> 
					<td><a href="viewInsect.jsp?bbsInsectID=<%=list.get(i).getBbsInsectID() %>">
					<%= list.get(i).getBbsInsectTitle() %></a></td>
					<!-- 제목 누르면 해당 게시글의 내용을 보여주는 페이지로 이동 -->
					<!-- 해당 게시글 번호를 매개변수로 보냄 -->
					<td><%= list.get(i).getUserID() %></td>
					<td><%= list.get(i).getBbsInsectDate().substring(0, 11) 
					+ list.get(i).getBbsInsectDate().substring(11, 13) 
					+ "시" + list.get(i).getBbsInsectDate().substring(14, 16) + "분" %></td>
					<!-- substirng: 필요한 부분만 잘라서 출력 -->
					<td><%= list.get(i).getBbsInsectHit() %></td>
				</tr>
				<%
					}
				%>
				...
				<%
					if (pageNumber != 1) {
				%>
					<a href="bbsInsect.jsp?pageNumber=<%=pageNumber - 1 %>" class="btn btn-success btn-arrow-left"
					style="background-color: #888686; border: #888686">이전</a>
				<%
					} if(bbsInsectDAO.nextPage(pageNumber + 1)) {  //다음 페이지가 존재한다면
				%>
					<a href="bbsInsect.jsp?pageNumber=<%=pageNumber + 1 %>" class="btn btn-success btn-arrow-right"
				...
				<c:if test="${userID eq '관리자' }">
					<a href="writeInsect.jsp" class="btn btn-primary pull-right" style="background-color: #b7b7b7; border-color: #b7b7b7;">글쓰기</a>
				</c:if> 
		</div>
	</div>
