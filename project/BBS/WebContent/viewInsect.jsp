		...
		<div class="row">  <!-- table-striped: 게시판의 글 목록들이 홀/짝수 번갈아가면서 색상 변경 -->
			...
			<tbody>
				<tr>
					...
					<td colspan="2"><%= bbsinsect.getBbsInsectTitle().replaceAll(" ", "&nbsp;")
				...
				<tr>
					...
					<td>조회수</td>
					<td style="width: 20%;"><%= bbsinsect.getBbsInsectHit() %></td>
				</tr>
				<tr>
					<td colspan="2">작성일자</td>
					<td colspan="2" style="width: 20%;"><%= bbsinsect.getBbsInsectDate().substring(0, 11) 
				...
				<tr>
					<td colspan="4" style="min-height: 200px; text-align: center; margin-left: auto; margin-right: auto;">
						<%= bbsinsect.getBbsInsectContent().replaceAll(" ", "&nbsp;")
						.replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br/>") %>
					</td>
					<!-- 특수문자 대체 대체 -->
				</tr>
			...
			<c:if test="${userID eq '관리자' }">
				<a href="updateInsect.jsp?bbsInsectID=<%= bbsInsectID %>" class = "btn btn-primary" style="background-color: #b7b7b7; border-color: #b7b7b7;">수정</a>
				<a onclick="return confirm('삭제하시겠습니까?')" href="deleteInsectAction.jsp?bbsInsectID=<%= bbsInsectID %>" class = "btn btn-primary"
				style="background-color: #b7b7b7; border-color: #b7b7b7;">삭제</a> 
			</c:if>
		</div>
	</div>
