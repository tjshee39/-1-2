...
<!-- 글 수정 -->
	...
	<%   //로그인 된 사람들의 정보를 담을 수 있도록 만듦
		...
		BbsPlant bbsplant = new BbsPlantDAO().getBbsPlant(bbsPlantID);  //넘어온 bbsID로 해당 글을 가져옴
		if (!userID.equals(bbsplant.getUserID())) {  //현재 유저가 글 작성자가 아니라면
			PrintWriter script = response.getWriter();
			script.println("<script>");
	...
	<nav class="navbar navbar-default">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				...
				<li class=""><a href="bbsPlant.jsp">식물</a>
	...
	<div class="container">  <!-- 특정한 내용을 담을 수 있는 container 지정 -->
		<div class="row">  <!-- table-striped: 게시판의 글 목록들이 홀/짝수 번갈아가면서 색상 변경 -->
			<form method="post" action="updatePlantAction.jsp?bbsPlantID=<%= bbsPlantID %>">  <!-- bbsID가 updateAction.jsp로 보내짐 -->
				<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
					...
					<tbody>
						<tr>
							<td><input type = "text" class="form-control" placeholder="글 제목" name="bbsPlantTitle" maxlength="50" 
							value=<%= bbsplant.getBbsPlantTitle() %>></td>  <!-- 수정하기 전의 제목 보여줌 -->
						</tr>	   
						<tr>
							<td><textarea class="form-control" placeholder="글 내용" name="bbsPlantContent" maxlength="2048" 
							style="height: 350px;"><%= bbsplant.getBbsPlantContent() %></textarea></td>
							<!-- 수정하기 전의 내용 보여줌 -->
						</tr>
					</tbody>
				</table>
